...
class ICMP(Protocol):
    ...
    ECHO_TYPES = (0, 8)  # echo request/reply use identifier + sequence
    ...
    def setup(self):
        ...
        if icmp_type.isdigit():
            ...
            if (new_type in self.ECHO_TYPES) != (
                self.header["type"] in self.ECHO_TYPES
            ):
    ...
    def deserializer(self, data: bytes):
        ...
        if self.header["type"] in self.ECHO_TYPES:
            self.header["identifier"], self.header["sequence"] = struct.unpack(
                "!HH", data[4:8]
            )
        else:
    ...
    def get_socket_info(self):
        # only matters standalone — nested under IPv4, IPv4 handles the raw socket
        import socket

...
class NTP(Protocol):
    ...
    def deserializer(self, packet: bytes):
        ...
        if mode in (3, 4):
            self.header["stratum"], self.header["poll"], self.header["precision"] = (
                struct.unpack("!BBb", packet[1:4])
            )
            ...
            self.reference_parameters["reference_timestamp"] = NTP.from_ntp_time(
                packet[16:24]
            )
    ...
    def __str__(self) -> str:
        ...
        if hasattr(self, "reference_parameters"):
            body += "\n" + self.format_table(
                "NTP Reference Parameters", self.reference_parameters
            )

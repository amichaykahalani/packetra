...
class IPv4(Protocol):
    ...
    def _build_payload(self) -> bytes:
        # UDP needs src/dst IP for its checksum, ICMP doesn't care
        ...
        if protocol_id == 17:  # UDP
            ...
            self.header["total_length"] = (
                self.header["IHL"] * 4 + self.payload.header["length"]
            )
        else:  # ICMP
            payload_bin = self.payload.to_binary()
    ...
    def build_header(self, checksum: int) -> bytes:
        ...
        header += struct.pack(
            "!2H", self.header["total_length"], self.header["identification"]
        )
        header += (
            (self.header["flags"] & 0x7) << 13 | (self.header["frag_offset"] & 0x1FFF)
        ).to_bytes(2, "big")
        header += struct.pack(
            "!2BH", self.header["TTL"], self.header["protocol"], checksum
        )
    ...
    def deserializer(self, data: bytes) -> Protocol:
        ...
        self.header["total_length"], self.header["identification"] = struct.unpack(
            "!2H", data[2:6]
        )
        ...
        self.header["TTL"], self.header["protocol"], self.header["checksum"] = (
            struct.unpack("!2BH", data[8:12])
        )
    ...
    def get_socket_info(self):
        # hand-built header + checksum, so this has to go out raw
        import socket

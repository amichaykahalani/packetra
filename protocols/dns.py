...
class DNS(Protocol):
    ...
    def __init__(self, domain: str = "", **kwargs):
        ...
        self.question_section = {
            "QNAME": domain,
            "QTYPE": self.RECORD_TYPES.get(kwargs.get('record_type'), self.RECORD_TYPES["IPv4"]),
            "QCLASS": self.RECORD_CLASSES.get(kwargs.get('class_type'), self.RECORD_CLASSES["IN"]),
        }
    ...
    def to_binary(self) -> bytes:
        ...
        packet += struct.pack('!HH', self.question_section['QTYPE'], self.question_section['QCLASS'])
    ...
    def _parse_header(self, packet: bytes) -> int:
        header_len = 12
        trans_id, flags, qdcount, ancount, nscount, arcount = struct.unpack('!HHHHHH', packet[:header_len])
        self.header = {
    ...
    def _decode_rdata(self, answer_type: int, packet: bytes, rdata_offset: int, rdata: bytes):
